...
from app.routers import pages, items, items_covers, items_csv, items_catalog, locations, platforms, settings, sync, checkouts, valuation, hardcover, store, series, share, tags, intake, archive, romm
...
async def check_loan_reminders() -> bool:
    ...
    with get_db() as db:
        ...
        last = db.execute("SELECT value FROM settings WHERE key = 'loan_reminder_last_sent'").fetchone()
        now = time.time()
    ...
    if not overdue:
        return False
    lines = [f"{o['title']} — {o['borrower_name']} ({o['days_out']} days out)" for o in overdue]
